...
def get_journal_by_type_action(action: str):
    return session.query(Journal).order_by(Journal.dateEnregistrement.desc()).filter(Journal.typeAction.contains(action)).all()
...
def get_all_journal():
    return session.query(Journal).order_by(Journal.dateEnregistrement.desc()).all()
...
def search_journals_by_date(search_date: datetime):
    ...
    result = session.query(Journal).order_by(Journal.dateEnregistrement.desc()).filter(
